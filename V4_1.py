...
async def on_ready():
    ...
    print(id)
    now = time.localtime()
    await client.get_channel(readylog).send("준홍봇 전원 on, %04d년%02d월%02d일 %02d시%02d분%02d초 online time." % (now.tm_year, now.tm_mon, now.tm_mday, now.tm_hour, now.tm_min, now.tm_sec))
    dagi = 8
